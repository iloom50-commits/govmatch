...
class AIService:
    ...
    async def extract_program_details(self, text: str) -> Dict[str, Any]:
        ...
        아래 텍스트가 기업/개인을 위한 정부 지원사업 공고인지 먼저 판별하고,
        지원사업이면 구조화하여 JSON으로 반환하세요.

        [지원사업 해당 기준 — is_support_program: true]
        - 기업/개인에게 자금·융자·보조금·인력·교육훈련·R&D·수출·판로 등을 지원하는 사업 모집 공고
        - 창업지원, 스타트업 육성, 소상공인 지원, 중소기업 지원 프로그램 모집

        [지원사업 아닌 경우 — is_support_program: false]
        - 기관 행사·포럼·세미나·간담회·설명회 개최 안내
        - 기관 직원/인턴 채용 공고
        - 민원 안내, 정보공개, 기관 소개 페이지
        - 수기 공모전, 사진 공모전 등 일반 공모
        - 교육 당첨자 명단, 합격자 발표
        - 통근버스, 시설 안내 등 내부 행정 정보
        ...
        [반환할 JSON 구조]
        {{
            "is_support_program": true 또는 false,
            "title": "공고 제목",
        ...
        is_support_program이 false이면 나머지 필드는 빈 값으로 두어도 됩니다.
