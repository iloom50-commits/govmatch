...
7. **첫 응답(messages 배열 길이 1~2)의 구조** — 반드시 아래 순서로 작성:
   ...
   ```
   ★★★ 본문에 "💡 추가로 물어볼 만한 것들" / "추가 질문" / "후속 질문" 같은 제목의 불릿 리스트를 절대 포함하지 마세요. 후속 질문은 반드시 `choices` JSON 필드로만 반환하세요. 프론트엔드가 choices를 클릭 가능한 버튼으로 렌더링합니다. ★★★
        ...
        try:
            ...
            def _analyze_announcement_realtime(ann_id: int) -> dict:
                ...
                try:
                    ...
                    try:
                        cur.execute("""
                            SELECT a.title, a.summary_text, aa.full_text,
                                   aa.parsed_sections, aa.deep_analysis
                            FROM announcements a
                            LEFT JOIN announcement_analysis aa ON a.announcement_id = aa.announcement_id
                            WHERE a.announcement_id = %s
                        ...
                        logger.warning(f"[analyze_realtime] query fail: {qe}")
                        try: db_conn.rollback()
                        except: pass
                        return {"error": f"조회 실패: {str(qe)[:100]}"}
                    if not r:
