...
def util_reset_plan(user=Depends(_get_current_user)):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("UPDATE users SET plan='free', billing_key=NULL, plan_expires_at=NULL WHERE user_id=%s", (user["user_id"],))
    conn.commit()
    cur.execute("SELECT * FROM users WHERE user_id=%s", (user["user_id"],))
    u = dict(cur.fetchone())
    conn.close()
    token = _create_jwt(u["user_id"], u["business_number"], u["email"], "free", None)
