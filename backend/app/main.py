...
### 소상공인 (에너지비용 지원)
- **대상**: 매출 기준 소상공인 (업종별 연매출 10억~120억 이하)
- **지원금액**: 전기·가스·유류비 실비 최대 **50만원**
- **신청처**: 소상공인진흥공단 (semas.or.kr) 또는 전국 소진공 지역센터 방문
- **준비서류**: 사업자등록증, 최근 3개월 에너지 요금 고지서, 통장사본

### 화물·버스·택시 운수업자 (유가연동보조금)
- **대상**: 사업용 화물차, 시내외버스, 택시 등록 사업자
- **지원방식**: 운행 km당 유류세 환급 (유가 상승분 연동)
- **화물차**: 국토교통부 화물서비스 시스템 (ftis.go.kr) 온라인 신청
- **버스·택시**: 관할 지방자치단체 운수사업과 방문 신청
- **지급주기**: 분기별 정산 후 지정 계좌 입금

### 취약계층 (에너지바우처)
- **대상**: 기초생활수급자, 차상위계층, 장애인·노인·영유아 가구
- **지원금액**: 연간 전기·도시가스·등유 **최대 30만원 상당** 바우처
- **신청처**: 주민센터 방문 또는 복지로 (bokjiro.go.kr) 온라인 신청
- **유효기간**: 지급연도 12월 31일까지 사용 (미사용분 소멸 주의)

## 지자체 추가 지원 확인 방법
- 거주지 시·군·구청 홈페이지 → 지역 에너지 지원사업 검색
- 서울: 서울에너지드림센터 (02-3455-5300)
- 경기: 경기에너지센터 (031-250-3566)

## 신청 시 유의사항
- **중복 수혜 불가**: 동일 유형 보조금은 1개 사업만 수령 가능
- **소득·매출 기준** 초과 시 환수 조치 있음
- **신청 기간**: 대부분 선착순 마감 → 공고 후 즉시 신청 권장""",
                    "에너지·소상공인",
                    "산업통상자원부·소진공·국토교통부",
...
# 충전팩 [결제금액(원), 지급 크레딧] — 소액 진입 우선(2026-07-17 확정).
# 첫 결제 허들을 낮추는 게 목적. 원가 방어선: 자금상담 100크레딧의 LLM 원가가 최악(캡15) ~75원이므로
# 크레딧당 0.7원 이상을 유지한다(10,000원팩 0.71원 = 최악 본전, 평균 8턴이면 마진 ~64%).
CREDIT_PACKS = json.loads(os.getenv(
    "CREDIT_PACKS",
    '[[1900,2000],[5000,6000],[10000,14000]]',
))
    ...
    try:
        ...
        def _simplify(a, idx):
            ...
            return {
                ...
                "deadline_type": d.get("deadline_type") or "",
...
# ── 관심 태그 풀 시드 & 임베딩 ──
# 시드(_INTEREST_TAG_SEED_BIZ / _INDIV)는 app.core.interest_tags 리프 모듈로 이관됨(작업3).
# matcher와 단일 출처를 공유하기 위함. 위 상단 import 참조.
...
async def api_blog_generate(req: BlogGenerateRequest):
    ...
    prompt = f"""당신은 네이버 블로그 SEO 전문가입니다.
아래 정부지원사업 공고를 바탕으로, 네이버 검색 상위에 노출되는 블로그 글을 작성해주세요.
네이버 상위 블로그를 벤치마킹하여, 실제 검색자가 신청 전 반드시 읽게 되는 완성도 높은 정보성 글을 써주세요.

[공고 정보]
- 사업명: {_sanitize(ann.get('title', ''), 80)}
- 지원내용: {_sanitize(ann.get('summary_text') or '', 800)}
- 지원대상: {_sanitize(ann.get('eligibility_logic') or '', 500)}
...
아래 형식으로만 응답하세요. 각 섹션은 ===구분자===로 시작합니다.

===TITLE===
검색자가 클릭할 제목 (50자 이내)
===CONTENT===
블로그 본문 전체 (자유롭게 작성, 2000자 이상)
===TAGS===
네이버 태그 10개 (쉼표 구분)
===META===
검색 스니펫용 한 줄 요약 (100자 이내)
===SEO===
핵심 SEO 키워드 3개 (쉼표 구분)"""
