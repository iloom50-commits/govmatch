  ...
  return (
    <div className="fixed inset-0 z-50 flex items-stretch justify-center lg:justify-end p-0 sm:p-2 lg:p-0 lg:pointer-events-none">
      ...
      >
        ...
        {/* Header — 드래그 핸들 */}
        <div
          className="relative z-10 px-4 py-4 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-violet-50 flex-shrink-0 cursor-grab active:cursor-grabbing select-none"
              ...
              <div className="min-w-0">
                ...
                {originUrl ? (
                  <a
                    ...
                    className="text-[13px] text-indigo-600 font-medium truncate flex items-center gap-1 hover:underline"
                  ...
                  <p className="text-[13px] text-slate-500 font-medium truncate">{announcement.title}</p>
                )}
              </div>
            </div>
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`${msg.role === "user" ? "max-w-[75%] order-1" : "max-w-[95%]"}`}>
                {/* Message bubble */}
                <div className={`rounded-2xl text-base md:text-[15px] leading-relaxed ${
                  msg.role === "user"
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {msg.choices.map((choice, ci) => (
                      <button
                        ...
                        disabled={loading}
                        className="px-3 py-2 bg-white border border-indigo-200 text-indigo-700 rounded-full text-sm md:text-[13px] font-semibold hover:bg-indigo-50 hover:border-indigo-300 transition-all active:scale-95 disabled:opacity-50"
                      >
                        {choice}
                      </button>
                    ))}
