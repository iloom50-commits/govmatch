    ...
    <div className="w-full max-w-[1280px] mx-auto animate-in fade-in duration-700 px-1 sm:px-2 lg:px-0 overflow-x-clip">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6 items-start">
        ...
        >
          ...
          <div className="grid grid-cols-4 border border-slate-200 mt-3 mb-3 bg-slate-50 rounded-lg overflow-hidden">
              {/* Row 1: ⭐맞춤 | 칩0 | 칩1 | 칩2 */}
              <button
              ...
              {(majorTab === "business" ? BUSINESS_CHIPS : INDIVIDUAL_CHIPS).slice(0, 3).map((chip, i) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center justify-center px-0 py-2.5 text-[13px] font-bold whitespace-nowrap transition-all border-b border-slate-200 active:scale-95 ${i < 2 ? "border-r" : ""} ${isActive ? activeColor : "text-slate-500 hover:bg-slate-100"}`}
                  >
                    {chip.label}
                  </button>
                );
              ...
              {(majorTab === "business" ? BUSINESS_CHIPS : INDIVIDUAL_CHIPS).slice(3, 6).map((chip, i) => {
                ...
                return (
                  <button
                    key={chip.key}
                    onClick={() => { setShowMatchedMode(false); setActiveChips(prev => { const n = new Set(prev); n.has(chip.key) ? n.delete(chip.key) : n.add(chip.key); return n; }); setCurrentPage(1); }}
                    className={`flex items-center justify-center px-0 py-2.5 text-[13px] font-bold whitespace-nowrap transition-all border-slate-200 active:scale-95 ${i < 2 ? "border-r" : ""} ${isActive ? activeColor : "text-slate-500 hover:bg-slate-100"}`}
                  >
                    {chip.label}
                  </button>
                );
