...
export default function ProfileSettings({ profile, onSave, onClose }: ProfileSettingsProps) {
  const [formData, setFormData] = useState({
    ...profile,
    revenue: profile.revenue_bracket || "UNDER_1B",
    employees: profile.employee_count_bracket || "UNDER_10",
  ...
  const revenueOptions = [
    { label: "1억 미만", value: "UNDER_1B" },
    { label: "1~5억", value: "1B_TO_5B" },
    { label: "5~10억", value: "5B_TO_10B" },
    { label: "10억 이상", value: "OVER_10B" }
  ];
  const employeeOptions = [
    { label: "5인 미만", value: "UNDER_5" },
    { label: "5~10인", value: "5_TO_10" },
    { label: "10~50인", value: "10_TO_50" },
    { label: "50인 이상", value: "OVER_50" }
  ];
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-end">
              ...
              {formData.industry_code && (
                <span className="text-[10px] font-black text-indigo-600 tracking-widest">KSIC {formData.industry_code}</span>
              )}
            </div>
            <div className="flex gap-2">
              ...
              <input
                ...
                placeholder="코드"
                maxLength={5}
                className="w-20 p-3 border border-slate-200 rounded-xl bg-white text-sm font-black outline-none text-center tracking-widest focus:ring-2 focus:ring-indigo-100"
            ...
            {industryCandidates.length > 0 && (
              <div className="space-y-1.5 max-h-[120px] overflow-y-auto">
                {industryCandidates.map((cand, idx) => (
                  <button
                    key={idx}
                    onClick={() => { setFormData({ ...formData, industry_code: cand.code }); setIndustryCandidates([]); setIndustryQuery(""); }}
