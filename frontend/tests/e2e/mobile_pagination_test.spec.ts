...
// ─── 칩 필터별 누적 검증용 헬퍼 ──────────────────────────────────────
async function testChipPagination(
  browser: any,
  chipLabel: string,
  useToken?: string,
): Promise<{ page1: number; afterScroll: number }> {
  const ctx = await browser.newContext({ viewport: MOBILE_VIEWPORT, isMobile: true });
  const page = await ctx.newPage();

  await page.goto(SITE);
  if (useToken) {
    await page.evaluate((t: string) => localStorage.setItem('auth_token', t), useToken);
    await page.reload();
  }
  await waitForCards(page, 45_000);
  ...
  // 칩 클릭 (label 기준) — 이모지 prefix 포함 가능 ("📍내지역" 등)
  const chip = page.locator('button').filter({ hasText: new RegExp(chipLabel.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')) }).first();
  await chip.waitFor({ timeout: 10_000 });
  await chip.click();
  console.log(`  → "${chipLabel}" 칩 클릭`);
  ...
  const page1Count = await countCards(page);
  console.log(`  → "${chipLabel}" 1페이지 공고 수: ${page1Count}건`);

  if (page1Count < 20) {
    console.log(`  ⚠️  ${page1Count}건 — 20건 미만, 2페이지 없음. 누적 테스트 스킵`);
    await ctx.close();
    return { page1: page1Count, afterScroll: page1Count };
  }
  ...
  await ctx.close();
  return { page1: page1Count, afterScroll: afterScrollCount };
}

// ═══════════════════════════════════════════════════════════════════════
// 테스트 4: "자금" 칩 모바일 무한스크롤 누적
// ═══════════════════════════════════════════════════════════════════════
test('"자금" 칩 선택 후 모바일 스크롤 시 공고가 누적된다', async ({ browser }) => {
  const { page1, afterScroll } = await testChipPagination(browser, '자금');

  if (page1 < 20) {
    // 데이터 부족으로 검증 불가 — 기능 자체 이상 여부만 확인
    expect(afterScroll).toBe(page1);
    console.log('  ⚠️  2페이지 없음 — 자금 카테고리 공고 부족, 현상 없음으로 판단');
    return;
  }

  expect(afterScroll).toBeGreaterThan(page1);
  console.log(`  ✅ "자금" 칩 누적 확인: ${page1}건 → ${afterScroll}건`);
});

// ═══════════════════════════════════════════════════════════════════════
// 테스트 5: "내 지역" 칩 모바일 무한스크롤 누적 (로그인 필요)
// ═══════════════════════════════════════════════════════════════════════
test('"내 지역" 칩 선택 후 모바일 스크롤 시 공고가 누적된다', async ({ browser }) => {
  // 내 지역 필터는 address_city가 있는 완성 프로필 계정 필요
  // 실제 버튼 텍스트: "📍내지역" (이모지 포함, 공백 없음)
  const { page1, afterScroll } = await testChipPagination(browser, '내지역', completeToken);

  if (page1 < 20) {
    expect(afterScroll).toBe(page1);
    console.log('  ⚠️  2페이지 없음 — 해당 지역 공고 부족, 현상 없음으로 판단');
    return;
  }

  expect(afterScroll).toBeGreaterThan(page1);
  console.log(`  ✅ "내 지역" 칩 누적 확인: ${page1}건 → ${afterScroll}건`);
});

// ═══════════════════════════════════════════════════════════════════════
// 테스트 6: "기술" 칩 모바일 무한스크롤 누적
// ═══════════════════════════════════════════════════════════════════════
test('"기술" 칩 선택 후 모바일 스크롤 시 공고가 누적된다', async ({ browser }) => {
  const { page1, afterScroll } = await testChipPagination(browser, '기술');

  if (page1 < 20) {
    expect(afterScroll).toBe(page1);
    console.log('  ⚠️  2페이지 없음 — 기술 카테고리 공고 부족, 현상 없음으로 판단');
    return;
  }

  expect(afterScroll).toBeGreaterThan(page1);
  console.log(`  ✅ "기술" 칩 누적 확인: ${page1}건 → ${afterScroll}건`);
});

// ═══════════════════════════════════════════════════════════════════════
// 테스트 7: 칩 전환 시 누적 데이터가 초기화되고 새 필터 결과 표시
// ═══════════════════════════════════════════════════════════════════════
test('칩 전환 시 이전 칩 누적 데이터가 초기화되고 새 칩 결과가 표시된다', async ({ browser }) => {
  const ctx = await browser.newContext({ viewport: MOBILE_VIEWPORT, isMobile: true });
  const page = await ctx.newPage();
  ...
  // 1) "자금" 칩 선택
  const fundChip = page.locator('button').filter({ hasText: /^자금$/ }).first();
  await fundChip.click();
  await page.waitForLoadState('networkidle', { timeout: 20_000 }).catch(() => {});
  await page.waitForFunction(() => !document.querySelector('.grid > div.animate-pulse'), { timeout: 20_000 }).catch(() => {});
  const fundCount = await countCards(page);
  console.log(`  → "자금" 칩 공고 수: ${fundCount}건`);

  // 2) 스크롤해서 페이지 2 로드
  let fundAfterScroll = fundCount;
  if (fundCount >= 20) {
    await scrollToSentinel(page);
    await page.waitForTimeout(3_000);
    fundAfterScroll = await countCards(page);
    console.log(`  → "자금" 스크롤 후: ${fundAfterScroll}건`);
  }

  // 3) "기술" 칩으로 전환
  const techChip = page.locator('button').filter({ hasText: /^기술$/ }).first();
  await techChip.click();
  await page.waitForLoadState('networkidle', { timeout: 20_000 }).catch(() => {});
  await page.waitForFunction(() => !document.querySelector('.grid > div.animate-pulse'), { timeout: 20_000 }).catch(() => {});
  await page.waitForTimeout(1_000);
  const techCount = await countCards(page);
  console.log(`  → "기술" 칩 전환 후 공고 수: ${techCount}건`);

  // 핵심 검증: 칩 전환 후 공고 수는 스크롤 누적 전인 자금 1페이지 첫 로드보다 적어야 함
  // (이전 "자금" 칩의 스크롤 누적 데이터가 남아있다면 fundAfterScroll 이상이 됨)
  // 단, 자금이 2페이지까지 갔을 경우에만 의미 있는 비교
  if (fundAfterScroll > fundCount) {
    expect(techCount).toBeLessThan(fundAfterScroll);
    console.log(`  ✅ 칩 전환 초기화 확인: ${techCount}건 < 자금 누적 ${fundAfterScroll}건 (이전 누적 없음)`);
  } else {
    // 자금 칩이 1페이지만 있었던 경우 — 단순히 데이터가 존재하는지만 확인
    expect(techCount).toBeGreaterThan(0);
    console.log(`  ✅ 칩 전환 후 공고 로드 확인: ${techCount}건`);
  }
