...
| 항목 | 상태 |
|------|------|
| 플랜 | trial(30일) / basic / expired | ✅ |
| trial_ends_at | 가입 시 +30일 저장 | ✅ |
| 만료 시 UI | 배너 "베이직 플랜 시작하기", 매칭 결과 비공개 | ✅ |
| D-7 이하 배너 | "업그레이드" 유도 | ✅ |
| 업그레이드 버튼 | 페이지 배너 + 사이드바 | ✅ |
| PaymentModal | 금액 2,900원 표시, 동의 후 `/api/plan/upgrade` 호출 | ✅ |
| 백엔드 upgrade | TOSS_SECRET_KEY 있으면 토스 결제 승인, 없으면 데모(즉시 basic 처리) | ✅ |
| 실제 PG 결제 UI | 토스페이먼츠 SDK 연동 **미구현**(결제창 호출 없음, 데모만 가능) | ⚠️ |
...
| 작업 | 내용 | 우선순위 |
|------|------|----------|
| **실제 결제 연동** | 토스페이먼츠(또는 PG) SDK로 결제창 호출 → paymentKey 확보 → `/api/plan/upgrade`에 전달. 현재는 데모(결제 없이 basic 전환)만 동작 | 높음 |
...
| 작업 | 내용 |
|------|------|
| **정기 결제(구독)** | 월 2,900원 자동 결제, 빌링키·결제 실패 재시도·해지 플로우 |
| **이메일 발송 검증** | SMTP 설정 시 다이제스트 실제 수신 테스트, 스팸 방지 |
| **에러 모니터링** | 백엔드/프론트 에러 로깅, 알림(선택) |
| **성능** | 매칭 결과 캐싱, 대량 공고 시 페이지네이션/가상 스크롤 |

### 3.3 선택 (기능 확장)

| 작업 | 내용 |
|------|------|
| **프리미엄 플랜** | 상위 요금제(예: AI 지원서 초안 등), 플랜별 기능 제한 |
| **컨설턴트 모드** | 별도 라우트/프로젝트로 분리 검토 (기업정보 입력 후 매칭 검색) |
...
- **목적**: 중소기업·소상공인 대상 AI 정부지원금 맞춤 매칭 + 알림 + 일정 관리, 월 2,900원 수익화.
- **개발현황**: 사용자 플로우(온보딩·로그인·매칭·프로필·결제 모달·캘린더·관리자)·백엔드 API·DB·AI·알림·E2E 테스트까지 구현 완료. **실제 PG 결제창만 미연동**(데모 결제만 가능).
- **남은 작업**: (1) 토스페이먼츠 등 실제 결제 연동, (2) 프로덕션 배포·환경 설정, (3) 정기 결제·해지 등 구독 로직, (4) 선택적으로 프리미엄·컨설턴트 모드·모니터링.
