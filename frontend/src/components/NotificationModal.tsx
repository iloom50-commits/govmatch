...
// ── 탭 버튼 공통 컴포넌트 ──
function Chip({ label, selected, onClick, color = "indigo" }: { label: string; selected: boolean; onClick: () => void; color?: "indigo" | "violet" }) {
  const cls = selected
    ? color === "violet"
  ...
  return (
    <button onClick={onClick} className={`px-4 py-2 rounded-full text-sm font-semibold border transition-all active:scale-95 ${cls}`}>
      {label}
    </button>
  );
}
...
export default function NotificationModal({
  ...
  const toggleInterest = (item: string) => setInterests(prev => prev.includes(item) ? prev.filter(i => i !== item) : [...prev, item]);
                ...
                {isBoth ? (
                  <>
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {IND_INTERESTS.map(item => <Chip key={item} label={item} selected={interests.includes(item)} onClick={() => toggleInterest(item)} />)}
                      </div>
                    </div>
                    <div>
                      <p className="text-sm font-bold text-violet-600 mb-2">기업 지원</p>
                      <div className="flex flex-wrap gap-2">
                        {BIZ_INTERESTS.map(item => <Chip key={item} label={item} selected={interests.includes(item)} onClick={() => toggleInterest(item)} color="violet" />)}
                      </div>
                    </div>
                  </>
                ) : (
                  <div>
                    <div className="flex flex-wrap gap-2">
                      {(isInd ? IND_INTERESTS : BIZ_INTERESTS).map(item => <Chip key={item} label={item} selected={interests.includes(item)} onClick={() => toggleInterest(item)} />)}
                    </div>
                  </div>
                )}
                {interests.length > 0 && <p className="text-xs text-indigo-500 font-semibold">{interests.length}개 선택됨</p>}
