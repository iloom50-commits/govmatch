...
def _render_fund_tables(funds) -> str:
    ...
    for f in funds:
        ...
        if not rows:
            continue
        blocks.append(
            f'<h4 style="margin:16px 0 4px 0;color:#5b21b6;font-size:13px;">{_esc(f.get("name") or "정책자금")}</h4>'
...
def build_fund_section_html(reply_text, announcements) -> str:
    ...
    if funds:
        rationale_block = f'<div style="margin-top:10px;">{_render_fund_tables(funds)}</div>'
    else:
