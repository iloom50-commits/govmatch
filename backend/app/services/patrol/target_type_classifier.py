...
분류 기준:
- business: 기업/법인/사업체가 주 신청 대상 (중소기업, 스타트업, 법인, 협동조합 등)
- individual: 개인이 주 신청 대상 (청년, 취업자, 구직자, 1인 가구, 임산부, 노인, 장애인, 학생, 저소득층 등 개인 자격으로 신청)
- both: 기업과 개인 모두 신청 가능 (소상공인 개인/법인 모두 가능, 창업자 개인/법인 모두 가능 등)

판단 팁:
- 소상공인: 개인사업자와 법인 모두 포함 → both
- 창업 초기: 팀/개인 창업자도 포함되면 → both
- R&D, 수출, 투자유치, 기업인증: 기업만 → business
- 청년창업: 개인 창업자 대상이면 → individual 또는 both (본문 맥락 판단)
- 주거지원, 복지급여, 장학금, 취업지원: → individual
...
def _classify_and_update(conn, cur, items: list[dict], label: Optional[str]) -> dict:
    ...
    try:
        type_map = _call_gemini_classify(items)
    except Exception as e:
        logger.error(f"[classifier] Gemini 호출 오류: {e}")
        # Gemini 실패 시 키워드 폴백
        type_map = _keyword_fallback([it["id"] for it in items],
